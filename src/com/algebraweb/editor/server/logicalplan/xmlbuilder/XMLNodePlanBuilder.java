...
public class XMLNodePlanBuilder {
	...
	private List<PlanNode> getAllNodesUnderThis(PlanNode rootNode,
			List<PlanNode> way) throws PlanHasCycleException {
		...
		while (it.hasNext()) {
			...
			wayCopy.addAll(way);
			if (wayCopy.contains(cur) && cur != null) {
	...
	private Map<Integer, Integer> getNodeIdReplacements(PlanNode rootNode,
			int offset) throws PlanHasCycleException {
	 ...
	 * 
	 * @param id
	 *            the id the XML plan should have
	 * @param rootNode
	 *            the root node of the plan
	 * @param c
	 *            the evaluation context of the plan
	 * @param context
	 *            the servlet context
	...
	public Element getNodePlan(int id, PlanNode rootNode, EvaluationContext c,
			ServletContext context) throws PlanManipulationException,
	...
	GraphIsEmptyException, PlanHasCycleException {
		...
		while (it.hasNext()) {
			...
			planBundle.addContent(getNodePlan(cur.getId(), cur.getRootNode(),
					cur.getEvContext(), context));
		}
	...
	/**
	 * Returns an XML representation of a single content node. All node ids are
	 * replaced according to the nodeIdReplacement map
	 * 
	 * @param n
	 *            the content node to translate
	 * @param nodeIdReplacements
	 *            the replacement map
	 * @return
	 */
	public Element getXMLElementFromContentNode(ContentNode n,
			Map<Integer, Integer> nodeIdReplacements) {
		Element ret = null;
		if (n instanceof PlanNode) {
			ret = new Element("node");
			if (nodeIdReplacements.get(((PlanNode) n).getId()) != null) {
				ret.setAttribute("id", Integer.toString(nodeIdReplacements
			...
			while (it.hasNext()) {
				...
				if (c.getPropertyVal().getType().equals("__NID")
						&& nodeIdReplacements.get(Integer.parseInt(c
								.getPropertyVal().getVal())) != null) {
					ret.setAttribute(c.getPropertyName(), Integer
							.toString(nodeIdReplacements.get(Integer.parseInt(c
									.getPropertyVal().getVal()))));
				} else {
					if (c.getPropertyVal().getVal() != null) {
						ret.setAttribute(c.getPropertyName(), c
								.getPropertyVal().getVal());
					}
				}
			}
			if (n instanceof ContentVal && ((ContentVal) n).getValue() != null
					&& ((ContentVal) n).getValue().getVal() != "") {
		...
		while (i.hasNext()) {
			ret.addContent(getXMLElementFromContentNode(i.next(),
					nodeIdReplacements));
		}
		return ret;
	}
