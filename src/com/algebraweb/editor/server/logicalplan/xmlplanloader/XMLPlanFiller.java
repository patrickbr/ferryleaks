...
/**
 * A GraphCanvasFiller for loading parsed xml files onto the client's
 * graphcanvas
 * 
...
public class XMLPlanFiller implements GraphCanvasFiller {
	...
	/**
	 * Translates a plan node into the raw node format that will be sent to the
	 * client
	 * 
	 * @param node
	...
	public RawNode getRawNode(PlanNode node) {
		RawNode temp = new RawNode(node.getId(), node.getLabel(), 0xCCCCCC,
				130, 25);
		...
		Map<String, NodeScheme> schemes = (Map<String, NodeScheme>) context
				.getAttribute("nodeSchemes");
		if (getScheme(node.getKind(), schemes).getProperties().containsKey(
				"color")) {
			temp.setColor(Integer.parseInt(getScheme(node.getKind(), schemes)
					.getProperties().get("color").split("x")[1], 16));
		}
		return temp;
	}
	...
	@Override
	public void init() {
		QueryPlanBundle qps = (QueryPlanBundle) session
