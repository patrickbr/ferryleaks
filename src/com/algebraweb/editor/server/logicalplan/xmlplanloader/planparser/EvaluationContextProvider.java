	...
	public void fillEvaluationContext(QueryPlan p)
		...
		if (root != null && root.getKind().equals("serialize relation")) {
			...
			if (root.getContentWithAttributeValue("function", "iter").size() > 0) {
				c.setIterColumnName(root.getContentWithAttributeValue(
						"function", "iter").get(0).getAttributes().get("name")
						.getVal());
			...
			if (root.getContentWithAttributeValue("function", "pos").size() > 0) {
				c.setSortColumnName(root.getContentWithAttributeValue(
						"function", "pos").get(0).getAttributes().get("name")
						.getVal());
			...
			Iterator<NodeContent> items = root.getContentWithAttributeValue(
					"function", "item").iterator();
			List<ItemCol> itemCols = new ArrayList<ItemCol>();

			while (items.hasNext()) {
				NodeContent current = items.next();
				itemCols.add(new ItemCol(current.getAttributes().get("name")
						.getVal(), Integer.parseInt(current.getAttributes()
						.get("position").getVal())));
			}
