 ...
 * Uses the browser standard scrolling-mechanisms, so scrolling will still
 * remain possible if mouse dragging isn't supported. (In that case, the widget
 * will be fully transparent.)
 * 
 * Childs are added using a standard FlowLayout (position:inline / float:left)
 * To prevent dragging for a specific period of time, use static preventDrag()
 * and unPreventDrag()
 * 
...
public class FullScreenDragPanel extends FlowPanel {
	...
	/**
	 * Prevent panel-dragging until unPreventDrag() is called. Since the
	 * drag-panel is designed to fill the whole browser-area, this is a static
	 * method.
	 */
	...
	private MouseMoveHandler mouseMoveHandler = new MouseMoveHandler() {
		...
		public void onMouseMove(MouseMoveEvent event) {
			if (mainDragOffsetX > -1 && !dragPreventer) {
				...
				FullScreenDragPanel.this.scrollTo(mainDragOffsetLeft
						- (event.getScreenX() - mainDragOffsetX),
						mainDragOffsetTop
								- (event.getScreenY() - mainDragOffsetY));
			} else if (dragPreventer) {
				FullScreenDragPanel.clearDrag();
			}
		}
	...
	public void center() {
		this.scrollTo(
				(this.getOffsetWidth() / 2 - Window.getClientWidth() / 2), this
						.getOffsetHeight()
						/ 2 - Window.getClientHeight() / 2);
	}

	public void center(int width, int height) {
		this.scrollTo((width / 2 - Window.getClientWidth() / 2), height / 2
				- Window.getClientHeight() / 2);
	}

	public void changeSavedScrollPos(int x, int y) {
		scrollPosition = new Tuple(x, y);
	}

	public void hide() {
		scrollPosition = new Tuple(Window.getScrollLeft(), Window
				.getScrollTop());
	...
	public void show() {
		...
		Window.scrollTo((int) scrollPosition.getX(), (int) scrollPosition
				.getY());
	}
