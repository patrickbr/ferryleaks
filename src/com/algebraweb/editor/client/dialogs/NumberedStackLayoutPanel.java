...
/**
 * A extension of GWT's StackLayoutPanel which enumerates the CSS-classes of the
 * stack items. It also adds a useful class "selected" when the stack is active.
 * 
...
public class NumberedStackLayoutPanel extends StackLayoutPanel {
	public NumberedStackLayoutPanel(Unit unit) {
		...
		super.addSelectionHandler(new SelectionHandler<Integer>() {
			...
			public void onSelection(SelectionEvent<Integer> event) {
				...
				getHeaderWidget(event.getSelectedItem()).addStyleName(
						"selected");
			}
		});
	}
	...
	@Override
	public void add(IsWidget widget, String header, boolean asHtml,
			double headerSize) {
	...
	@Override
	public void add(Widget widget, String header, boolean asHtml,
			double headerSize) {
		super.add(widget, header, asHtml, headerSize);
		updateClass();
	}
	...
	private void updateClass() {
		if (super.getWidgetCount() <= 1) {
			...
			super.getHeaderWidget(super.getWidgetCount() - 2).removeStyleName(
					"last");
			super.getHeaderWidget(super.getWidgetCount() - 1).addStyleName(
					"last");
		}
