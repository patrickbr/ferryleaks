...
public class PropertyEditPanel extends Composite {
	...
	public PropertyEditPanel(int pid, ContentNodeTreeItem treeItem,
			RemoteManipulationServiceAsync manServ, NodeContent c,
			GoAble scheme, PlanNode nodeContext) {
		...
		if (this.scheme.hasFields()) {
			...
			while (it.hasNext()) {
				...
				if (c.getAttributes().containsKey(current.getName())) {
					...
					pv = c.getAttributes().put(current.getName(),
				...
				if (current.getType().matches("__COLUMN[\\{]?[0-9]*[\\}]?")
						|| current.getType().equals("__COLUMN_REMOVE")) {
					f = new PropertyEditFieldAvailableColumns(pid, nodeContext
							.getId(), manServ, current);
		...
		if (this.scheme instanceof Value && c instanceof ContentVal
				&& ((Value) this.scheme).hasVal()) {
